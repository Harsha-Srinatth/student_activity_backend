...
export const getDepartmentFaculty = async (req, res) => {
  try {
    ...
    // Get faculty in HOD's specific department
    const faculty = await FacultyDetails.find({ 
      collegeId,
      dept: department 
    })
      .select("facultyid fullname email dept designation mobile")
      .sort("fullname")
      .lean();
    ...
    return res.status(200).json({
      ...
      data: faculty.map(f => ({
        ...
        specialization: f.designation || null,
        phone: f.mobile || null,
      })),
    });
  } catch (error) {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      data: students.map(s => ({
        ...
        year: Math.ceil((parseInt(s.semester) || 1) / 2),
