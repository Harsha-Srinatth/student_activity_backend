...
const getEnrollments = async (req, res) => {
  try {
    const student = await StudentDetails.findOne({ studentid: req.user.studentid });
    if (!student) {
...
const enrollInClub = async (req, res) => {
   try {
    ...
    const { clubId, clubName, studentName, regno, branch, section, dept, phone, email } = req.body;

    // Check duplicate request
    const exists = await StudentDetails.enrollments.findOne({ regno: studentId, clubId: clubId });
    if (exists) {
